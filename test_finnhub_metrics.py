...
# Список технологических компаний с высокой ликвидностью для тестирования
tech_tickers = ["AAPL", "MSFT", "GOOGL", "AMZN", "META", "TSLA", "NVDA"]
print(f"\nТестирование расчета метрик для технологических компаний: {', '.join(tech_tickers)}")

for ticker in tech_tickers:
    try:
        # Удаляем кэш для тикера перед тестированием
        cache_file = cache_dir / f"{ticker}.json"
        if cache_file.exists():
            cache_file.unlink()
            print(f"Кэш для {ticker} удален.")
